...
from afdme.utils import generate_sinusoid_tile
from afdme.filter.dft import DFTFilter
...
def main():
    ...
    writer = iio.get_writer(
        str(image_path) + "/demo_sine.gif", mode="I", fps=int(1000 / fps)
    )
    for i in range(n):
    ...
    writer = iio.get_writer(
        str(image_path) + "/demo_filtered_sine.gif", mode="I", fps=fps
    )
    for i in range(n):
