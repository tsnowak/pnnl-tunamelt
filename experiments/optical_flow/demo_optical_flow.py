...
from fish.filter.lucas_kanade import lucas_kanade
...
if __name__ == "__main__":
    ...
    for i in range(s_channel.shape[0]):
        background_subtracted[i, :, :, 0] = background_subtracted[i, :, :, 0]*motion_mask


    intensity_filter = intensity_filter(background_subtracted)

    logger.info("Writing Demos to file")
    raw_writer = iio.get_writer(str(image_path) + '/demo_raw.gif',
                                mode='I', fps=fps)
    filter_writer = iio.get_writer(str(image_path) + '/demo_filtered.gif',
                                   mode='I', fps=fps)
