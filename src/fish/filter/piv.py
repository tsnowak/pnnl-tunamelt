...
# save to a file
    ...
    for i in range(numFrames):
        ...
        u_buff, v_buff, mask_buff = validation.global_val( u_buff, v_buff, (-300.,300.),(-300.,300.))
        u_buff, v_buff, mask_buff = validation.sig2noise_val(u_buff, v_buff, sig2noise, threshold = 1.1 )
        u_buff, v_buff = filters.replace_outliers( u_buff, v_buff, method='localmean', max_iter = 3, kernel_size = 3)
        x_buff, y_buff, u_buff, v_buff = scaling.uniform(x, y, u_buff, v_buff, scaling_factor = scaling_factor )
        x_buff, y_buff, u_buff, v_buff = tools.transform_coordinates(x, y, u_buff, v_buff)
        u += u_buff
        v += v_buff
    ...
    tools.display_vector_field('test.txt', ax=ax, scaling_factor=scaling_factor, scale=5, width=0.0035, on_img=True, image_name='frame_a.png')
