...
def lucas_kanade(video: Array["N,H,W,C", np.uint8],
               fps: int,
    ...
    feature_params = dict(maxCorners=100, qualityLevel=0.00000001, minDistance=2, blockSize=2)

    lk_params = dict(winSize = (60,80), maxLevel=8, criteria=(cv2.TERM_CRITERIA_EPS | 
                cv2.TERM_CRITERIA_COUNT, 60, 0.03), minEigThreshold = 1e-8)
    ...
    mask = np.zeros_like(np.dstack((video[0,:,:,:], np.zeros((video.shape[1], video.shape[2], 2)))), dtype=np.uint8)
    motion_mask = mask
     
    p1, st, err = cv2.calcOpticalFlowPyrLK(old_gray, video[8,:,:,0] , p0, None, **lk_params)

    motion_filter = iio.get_writer('motion_filter.gif', mode='I', fps=fps)
    i = 9
    while i<video.shape[0]:
        
        frame = np.dstack((video[i, :, :, :], np.zeros((video.shape[1], video.shape[2], 2))))
        #print(frame.shape)
    ...
    logger.debug(motion_mask.shape)
