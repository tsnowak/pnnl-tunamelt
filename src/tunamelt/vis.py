...
from tunamelt.data import label_to_per_frame_list, xywh_to_xyxy
...
def write_video(
    ...
    assert isinstance(video_length, int), "Video length is not an integer value"

    full_file_name = str(out_path) + "/" + f"{name}{video_type}"
    log.info(f"Started writing {full_file_name}")
    writer = iio.get_writer(full_file_name, mode="I", fps=fps)
    ...
    writer.close()
    log.info(f"Finished writing {full_file_name}")
