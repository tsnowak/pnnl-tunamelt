...
import tunamelt.filter
from tunamelt import REPO_PATH, log
from tunamelt.filter import common, dft
from tunamelt.data import DataLoader, Dataset, numpy_to_cv2
from tunamelt.vis import viz_video_results, write_results
from tunamelt.utils import create_exp_dirs, load_params, generate_param_batches
...
def create_pipeline(args, params, fps):
    ...
    for filter in args["filters"]:
        try:
            filter_func = locals()[f"{filter}"]
            filters.update({filter: filter_func})
        except SyntaxError:
            raise SyntaxError(
