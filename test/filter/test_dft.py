...
def test_filter_frequency():
    ...
    filter_freq_range = (0.5, 9)
    element_shape = (10, 15)
    n_frames = 1000

    waveform, fps, length = generate_sinusoid_tile(
        freqs=freqs, element_shape=element_shape, n_frames=n_frames
    )
    ...
    dft = DFTFilter(freq_range=filter_freq_range)
    ...
    freqs_in_range = (freqs <= filter_freq_range[1]) * (freqs >= filter_freq_range[0])
    # Get the indices in the waveform that contain these frequencies
    indices = np.squeeze(np.argwhere(freqs_in_range is True))
    # step through waveform bins and verify pixels are correctly masked
    for i in range(len(freqs)):
        bin_is_one = np.all(
            mask[:, (element_shape[1] * i) : (element_shape[1] * (i + 1)), :] is True
        )
        log.debug(f"Mask value for waveform bin {i} equals 1? {bin_is_one}")
        assert (bin_is_one) == (
            i in indices
    ...
    log.info("DFT filter is working properly.")
