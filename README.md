...
└── mp4
    ...
    └── train
```
Inside directories under `labels` are `.xml` files generated by (CVAT)[https://github.com/cvat-ai/cvat] which contain the per-frame bounding boxes. Inside the directories under `mp4` are `.mp4` files of the acoustic camera videos. Test video files were batched into video files of up to 300 frames for processing. 
